...
import static ai.kitt.snowboy.Constants.BACK_UMDL;
import static ai.kitt.snowboy.Constants.NEXT_UMDL;
...
public class RecordingThread {
    ...
    private String activeModel = strEnvWorkSpace+ACTIVE_UMDL; //+","+strEnvWorkSpace+BACK_UMDL;
    ...
    public RecordingThread(Handler handler, AudioDataReceivedListener listener) {
        ...
        detector.SetSensitivity("0.6"); //,0.6
    ...
    private void sendMessage(MsgEnum what,int result){
        if (null != handler) {
            Message msg = handler.obtainMessage(what.ordinal(), null);
    ...
    private void record() {
        ...
        while (shouldContinue) {
            ...
            if (result == -2) {
                ...
                sendMessage(MsgEnum.MSG_ERROR, result);
            } else if (result == 0) {
                // post a higher CPU usage:
